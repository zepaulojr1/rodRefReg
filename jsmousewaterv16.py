...
#ISSUE: PROGRAM WONT CLOSE WHEN YOU ONLY DECLARE ONE HAT
#ISSUE: IMMENSE LAG
...
def initialize_relays(num_hats):
    relays = []
    failed_hats = []
    for i in range(num_hats):
        try:
            relays.append(sm_16relind.SM16relind(stack=i))
        except Exception as e:
            print (f"{datetime.datetime.now()} - Error initializing hat #{i + 1}: {e}")
            failed_hats.append(i + 1)
    if failed_hats:
#        print_to_terminal(f"Failed to initialize hats: {', '.join(map(str, failed_hats))}")
        return relays

num_hats = ask_number_of_hats()
relays = initialize_relays(num_hats)
...
def stop_program():
    ...
    for hat_index in range(num_hats):
        set_all_relays(hat_index, 0)  # This closes all relays when stopping program
    root.destroy()
