...
# Function to add styled text to subheaders (in the scrollable frame)
# Uses tkinter text function
def add_styled_text(frame, text, row, bold_indices=None, font=("Arial", 12)):
	text_widget = tk.Text(frame, wrap='word', font=font, height=3, bg=frame.cget("bg"), bd=0, highlightthickness=0)
	text_widget.insert("1.0", text)

	if bold_indices:
		for start, end in bold_indices:
			text_widget.tag_add("bold", f"1.{start}", f"1.{end}")

	text_widget.tag_configure("bold", font=(font[0], font[1], "bold"))
	text_widget.config(state="disabled")
	text_widget.grid(row=row, column=0, columnspan=2, pady=(0, 5), sticky="w")
...
# Creating a canvas (So we can create a scrollbar for the rest of the GUI)
canvas = Canvas(root)
scrollbar = Scrollbar(root, orient="vertical", command=canvas.yview)
scrollable_frame = Frame(canvas)
scrollbar.pack(side="right", fill="y") # To pack the scrollbar next to the canvas, and only up to the terminal scrollbar
canvas.pack(side="left", fill="both", expand=True)

# This tells the canvas to watch for updates on the scrollable_frame's size
...
canvas.create_window((0, 0), window=scrollable_frame, anchor="nw") # Pack the scrollable frame into the canvas window
canvas.configure(yscrollcommand=scrollbar.set)

## I will now assign all GUI elements to "scrollable_frame" instead of root, so that everything moves according to the scrollbar.

# GUI Welcome message and subheaders
welcome_label = tk.Label(scrollable_frame, text="Welcome to the Rodent Refreshment Regulator Setup Wizard", font=("Arial", 16))
welcome_label.pack(pady=(10, 5))

add_styled_text(scrollable_frame, "Step 1: Answer the following questions to suit your needs.",
	bold_indices=[(0, 6)], font=("Arial", 12)) #"bold the first 6 char"
add_styled_text(scrollable_frame, "Step 2: Press the 'Suggest Settings' button to receive setting recommendations in the terminal below.",
	bold_indices=[(0, 6)], font=("Arial", 12))
add_styled_text(scrollable_frame, "Step 3: Adjust settings as needed, and press the 'Push Settings' button to submit and save your settings",
	bold_indices=[(0, 6)], font=("Arial", 12))
add_styled_text(scrollable_frame, "Notes:\n * You may manually adjust settings within the dropdown menu below\n * Questions pertaining to water volume are for EACH relay",
	bold_indices=[(0, 5)], font=("Arial", 12))

# Frames for the Qs and User Answers
questions_frame = ttk.Frame(scrollable_frame)
questions_frame.grid(row=0, column=1, rowspan=6, sticky="ne", padx=20)
...
questions = [
	...
	"How often (in seconds) should each cage to receive this amount of water?",
...
# I'm creating this section fo work on a way for relay pairs to be given a different number of triggers by the user
...
# Push Settings Function (based on the user inputs to the Qs at the top of the GUI
...
bottom_container = tk.Frame(scrollable_frame)
