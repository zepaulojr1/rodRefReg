...
welcome_label = tk.Label(left_content_frame, text="Welcome to the Rodent Refreshment Regulator Setup Wizard", font=("Arial", 24, "bold"))
welcome_label.pack(fill='x', padx=10, pady=1)
...
def add_styled_text_single(left_content_frame, text):
	...
	text_frame.pack(fill='x', padx=(25,10), pady=(5, 5))

	text_widget = tk.Text(text_frame, wrap='word', height=15, font=("Arial", 12),
...
#note: (\s) is for spaces, (\ b) is a boundary marker to distinguish words within words (not in use anymore)
	...
	for pattern in bold_patterns:
		...
		while True:
			...
			length_of_match = len(text_widget.get(start_index, f"{start_index} lineend"))
...
questions_frame = tk.LabelFrame(right_content_frame, text="Answer these to receive setting stuggestions", font=("Arial", 12, "bold"))
...
buttons_frame.pack(fill='x', padx=5, pady=(25, 25))
