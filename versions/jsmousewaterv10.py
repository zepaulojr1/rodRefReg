#RODENT REFRESHMENT REGULATOR VERSION 0.9
#Last updated 17/05/2024 by JS
# Going to version 10. Still have some minor gui issues, but will be fixing suggest/push. Still need to make email system customizable too.
...
def toggle_relay(relay_pair):  # To turn relay pairs on/off
...
def create_trigger_entries():
	...
	trigger_entries = {}
	# Remember to update num_columns as needed for proper gui layout etc.
	for i, relay_pair in enumerate(RELAY_PAIRS):
		...
		# adds padding and align to the left for proper UI fitting. padx is responsible for adding horizontal space between the label and entry field.
...
def get_trigger_counts(): # To assign chosen number of triggers for each RP
	trigger_counts = {}
	for relay_pair, entry in trigger_entries.items():
		try:
			count = int(entry.get())
...
#scrollable_frame.grid_rowconfigure(0, pad=10)  # Reduce padding for the title row if necessary
#scrollable_frame.grid_rowconfigure(1, pad=10)  # Reduce padding for the wsubheader row

scrollable_frame.grid_rowconfigure(0, minsize=30)  # for the welcome header
scrollable_frame.grid_rowconfigure(1, minsize=5)  # Reduce the minsize for subheaders to reduce space
scrollable_frame.grid_rowconfigure(2, minsize=10)  # for the button bar
scrollable_frame.grid_rowconfigure(3, minsize=20)  # for the advanced settings header



# Adding more rows to scrollable_frame, to allow for better GUI widget placement
#for i in range(20): # adjusting to 20 rows (from 10 in v8)
#	scrollable_frame.grid_rowconfigure(i, weight=1 if i in [1, 4] else 0, minsize=30 if i in [1, 4] else 10)
# the above line helps manage spacing and stability when opening closing the advanced settings menu.
        # DO NOT MESS WITH! IT TOOK FOREVER TO GET THIS TO WORK


# Configuring columns for scrollable frames (should hopefully fix gui widget overlap issue I'm having)
scrollable_frame.columnconfigure(5, weight=1, minsize=500)  # Allocating more weight to columns that contain user questions and pictures to fix overlap

# If scrollable_frame spans multiple columns, make sure it's configured properly
#root.columnconfigure(0, weight=1)  # Configure scrollable_frame to span multiple columns (may be redundant now)
...
welcome_label = tk.Label(scrollable_frame, text="Welcome to the Rodent Refreshment Regulator Setup Wizard", font=("Arial", 24, "bold"))
welcome_label.grid(row=0, column=0, columnspan=3, sticky="w", padx=10, pady=2)
welcome_label.grid_propagate(False) # Prevents the frame from changing its size as I change other things
...
# Function for adding styled text (i.e., bolding) to the subheaders etc. (uses "text" through tkinter)
def add_styled_text_single(frame, text):
	text_widget = tk.Text(frame, wrap='word', font=("Arial", 12), bg=frame.cget("bg"), bd=0, highlightthickness=0) #ADJUST HEIGHT TO PREVENT CUT-OFF
	text_widget.grid(row=1, column=0, columnspan=5, sticky="ew", padx=(10,0))
	text_widget.insert("1.0", text) #insert text into widget
	text_widget.tag_configure("bold", font=("Arial", 12, "bold")) #bolding

	bold_patterns = [
		r"Step \d+:", # Find and bold "Step 1:", "Step 2:", and "Step 3:"
		r"Notes:",    # Find and bold "Notes:"
		r"EACH",      # Find and bold "EACH"
		r"UP",        # Find and bold "UP"
		r"\*",         # Find and bold all instances of "*"
		r"CTRL+C"
	]

	# Apply the bold tag to the bolded sections specified above in bold_patterns
	for pattern in bold_patterns:
		for match in re.finditer(pattern, text):
			newline_count = text.count('\n', 0, match.start()) # find line num. for each set of chars that need bolding
			line_start = newline_count + 1
			line_char_start = match.start() - text.rfind('\n', 0, match.start()) - 1 # "-1" is to include char. 1
			start = f"{line_start}.{line_char_start}" # find start pos. for bolding
			end = f"{line_start}.{line_char_start + len(match.group(0))}" # find end pos. for bolding
...
add_styled_text_single(scrollable_frame, subheaders_text)
...
questions_frame.grid(row=1, column=4, columnspan=2, sticky="ne", padx=20, pady=0)
...
buttons_frame.grid(row=4, column=4, sticky="nw", pady=(0, 10))
...
push_button.grid(row=0, column=1, padx=5, pady=5, sticky="w")
run_button.grid(row=0, column=2, padx=5, pady=5, sticky="w")
stop_button.grid(row=0, column=3, padx=5, pady=5, sticky="w")
...
advanced_settings_frame = tk.Frame(scrollable_frame)
advanced_settings_frame = tk.LabelFrame(scrollable_frame, text="Advanced Settings", font=("Arial", 12, "bold"))
advanced_settings_frame.grid(row=3, column=0, columnspan=5, sticky='nsew', padx=10, pady=10)
advanced_settings_frame.columnconfigure(0, weight=1)  #change weight if more columns are needed to show stuff
...
create_trigger_entries() # for use creating trigger entires UI
...
def create_trigger_entries():
	...
	for i, relay_pair in enumerate(RELAY_PAIRS):
		...
		frame.grid(row=8, column=i % 4, padx=10, pady=(0, 10), sticky="w")
...
settings_frame.grid(row=9, column=1, padx=10, pady=(10, 0))

# UI for specifying the interval
interval_frame = tk.Frame(advanced_settings_frame)
interval_frame.grid(row=10, column=0, padx=10, pady=(5, 0))
...
update_settings_button.grid(row=13, column=0, sticky="w", padx=10, pady=10)  #Change Columnspan if too wide
...
######################################################
#ROCK STUFF CURRENTLY DISABLED (TURNED INTO COMMENTS)#
######################################################
...
image_frame.grid(row=0, column=5, sticky="ne", padx=10, pady=10)
...
left_image_label.grid(row=0, column=0, padx=20, sticky="w")
