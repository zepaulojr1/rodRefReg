#RODENT REFRESHMENT REGULATOR VERSION 14
#Last updated 29/05/2024 by JS
...
# Create a text widget with a vertical scrollbar
	...
	text_frame.pack(fill='x', padx=(25,10), pady=(1, 1))
...
# Remember to update num_columns as needed for proper gui layout etc.
	for i, relay_pair in enumerate(RELAY_PAIRS):
		...
		var = tk.BooleanVar(value=True)  # Ensures default is True
		chk = tk.Checkbutton(frame, text=f"Enable {RELAY_NAMES[relay_pair]}", variable=var, onvalue=True, offvalue=False) #checkboxes for each rp
		...
		trigger_entry.insert(0, "0")  # Default trigger count to 0
		trigger_entry.pack(side='left')
		trigger_entries[relay_pair] = trigger_entry #again storing widget in dict.

create_trigger_entries() # for use creating trigger entires UI (redundant?? investigate later)
...
#Easier way to put everything into the advanced settings frame (turned lines into comments above - delete later if this works)
# remembber to ncrease row index for each subsequent setting
#
#def setup_interval_stagger_entries(advanced_settings_frame):
#	# Labels
#	settings_labels = [
#		"Interval (seconds):",
#		"Stagger (seconds):",
#		"Water Window Start (24-hour format):",
#		"Water Window End (24-hour format):"
#	]
#	settings_entries = [interval_entry, stagger_entry, window_start_entry, window_end_entry]
#
#	# Ensure the entry widgets are global (since I changed how they are initialized above - may not be needed)
##	global interval_entry, stagger_entry, window_start_entry, window_end_entry
#
#	# Entry field creation
#	interval_entry = tk.Entry(advanced_settings_frame)
#	interval_entry.insert(0, INTERVAL)
#	stagger_entry = tk.Entry(advanced_settings_frame)
#	stagger_entry.insert(0, STAGGER)
#	window_start_entry = tk.Entry(advanced_settings_frame)
#	window_start_entry.insert(0, WINDOW_START)
#	window_end_entry = tk.Entry(advanced_settings_frame)
#	window_end_entry.insert(0, WINDOW_END)
#
#	# Grid settings for the labels and entries
#	for index, (label_text, entry_widget) in enumerate(zip(settings_labels, settings_entries)):
##		tk.Label(advanced_settings_frame, text=label_text).grid(row=index, column=0, sticky="w", padx=5, pady=5)
#		entry_widget.grid(row=index, column=1, sticky="w", padx=5, pady=5)
#
#setup_interval_stagger_entries(advanced_settings_frame)
...
left_image_label.grid(row=0, column=0, padx=(50,10), sticky="w")
