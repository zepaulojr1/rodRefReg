...
##NOTE: I'm not sure if this is still needed now that I'm not using the GPIO pins. Experiment without this later
				...
				if current_time % INTERVAL < 1:  # Determining whether it's time to trigger the pumps (based on the interval)
					for relay_pair in RELAY_PAIRS:  # Go through all relay pairs, not just the selected ones
						if relay_pair in selected_relays:  # Now only sned a trigger if the relay pair is selected (should fix the tickbox problem)
							...
							time.sleep(1) # Checks again every second
					time.sleep(1)
...
def on_close(): #so that the program stops when exiting the gui, instead of running in the background
        stop_program()
...
# Adding a button to update the number of triggers
update_button = tk.Button(toggle_frame, text="Update Number of Triggers", command=update_trigger_counts)
update_button.grid(row=(len(RELAY_PAIRS) // num_columns) * 2 + 1, columnspan=num_columns, pady=10) # The +1  makes sure the update button doesn't overlap with the trigger textboxes (which take up 2 rows)
...
tk.Button(interval_frame, text="Apply", command=update_interval).grid(row=0, column=2, sticky="w")  # Aligns the button to the left
...
stagger_entry.grid(row=0, column=1, sticky="w")  # Aligns left
tk.Button(stagger_frame, text="Apply", command=update_stagger).grid(row=0, column=2, sticky="w")  # Aligns left
...
tk.Button(window_frame, text="Apply", command=update_window).grid(row=1, column=2, sticky="w")
...
warning_message2 = "An email will be sent to () upon each successful pump trigger. CTRL+C force closes the program if needed."
