...
LOG_FILE = "pump_log.json" # File to store the log of pump triggers
...
def send_slack_notification(message):
    try:
        ...
        print_to_terminal(f"{datetime.datetime.now()} - Slack message sent successfully! Response: {response}")
    except SlackApiError as e:
        print(f"{datetime.datetime.now()} - Failed to send Slack message: {e.response['error']}")
        print_to_terminal(f"{datetime.datetime.now()} - Failed to send Slack message: {e.response['error']}")
        log_pump_trigger(message)
# Replace 'your-channel-id' with the actual Slack channel ID
channel_id = "your-channel-id"
...
def log_pump_trigger(relay_info):
    log_entry = {
    ...
    print_to_terminal(f"Logged pump trigger: {log_entry}")

def retry_sending_logs():
    while True:
        if is_internet_available() and os.path.exists(LOG_FILE):
            with open(LOG_FILE, 'r') as f:
                logs = json.load(f)
...
def start_retry_thread():
    retry_thread = threading.Thread(target=retry_sending_logs)
...
def program_loop():
    ...
    try:
        while running:
            current_hour = time.localtime().tm_hour  # Retrieve the current hour
            if (WINDOW_START <= current_hour < 24) or (0 <= current_hour < WINDOW_END) if WINDOW_START > WINDOW_END else (WINDOW_START <= current_hour < WINDOW_END):
                ...
                if current_time % INTERVAL < 1:  # Determine whether it's time to trigger the pumps (based on the interval)
                    relay_info = []  # Gather information for the Slack notification
                    for relay_pair in RELAY_PAIRS:
                        if relay_pair in selected_relays:
                            triggers = num_triggers.get(relay_pair, 1)
                            for _ in range(triggers):
                                relay1, relay2 = relay_pair
                                rel.set(relay1, 1)  # Turn on the first relay in the pair
                                rel.set(relay2, 1)  # Turn on the second relay in the pair
                                ...
                                time.sleep(STAGGER)
                                rel.set(relay1, 0)  # Turn off the first relay in the pair
                                rel.set(relay2, 0)  # Turn off the second relay in the pair
                    ...
                    time.sleep(INTERVAL - 1)  # Adjust for the time already spent triggering relays
        ...
        rel.set_all(0)  # Turn off all relays when the program exits
...
# Scaleable way of creating toggle and trigger settings that can receive input from the setup page
...
# ______________________________________________________________________________________________________________________________________________________________________________________________
# __________PROGRAM_CLOSUREAND_OTHER_END_STUFF_________________________________________________________________________________________________________________________________________________
# ______________________________________________________________________________________________________________________________________________________________________________________________
