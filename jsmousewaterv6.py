...
info_label = tk.Label(scrollable_frame, text="Steps: (1) r", font=("Arial", 12))  # !!!placeholder, fill in later!
info_label.pack()

info_label2 = tk.Label(scrollable_frame, text="placeholder", font=("Arial", 12))  # !! placeholder again
info_label2.pack()

# Frames for the Qs and User Answers
questions_frame = ttk.Frame(scrollable_frame)
questions_frame.pack(fill="x", expand=True, padx=20, pady=20)
...
for i, question in enumerate(questions):
	row = i // 2 # to arrange questions into columns. Starting with 2 for now
	col = i % 2

	label = tk.Label(questions_frame, text=question)
	label.grid(row=row, column=col*2, sticky="e", padx=5, pady=2)  # Even columns for labels

	entry = tk.Entry(questions_frame, width=20)
	entry.insert(0, "0") #Default each entry to 0 
	entry.grid(row=row, column=col*2 + 1, sticky="w", padx=5, pady=2)  # Odd columns for entries
	entries[question] = entry

questions_frame.columnconfigure(0, weight=1) # These lines ensure the Q&As allocate horizontal space in the GUI correctly
questions_frame.columnconfigure(1, weight=1)
questions_frame.columnconfigure(2, weight=1)
questions_frame.columnconfigure(3, weight=1)
...
suggest_button = tk.Button(scrollable_frame, text="Suggest Settings", command=suggest_settings)
suggest_button.pack(pady=(10, 5))

push_button = tk.Button(scrollable_frame, text="Push Settings", command=push_settings)
push_button.pack()
